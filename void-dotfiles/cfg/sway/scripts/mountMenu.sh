...
label="$(find $DEV_LABEL* | cut -d'/' -f5 | wmenu -il 3 -f "Maple Mono NF 11" -N 0d0e0f -n ebdbb2 -M 689d6a -m 0d0e0f -S 689d6a -s 0d0e0f -p "MOUNT MENU: ")"
path="$DEV_LABEL$label"
[[ -z $label ]] && echo "Cancelled." && exit

action=$(echo -e "MOUNT\nUNMOUNT\nPOWEROFF" | wmenu -il 3 -f "MapleMono NF 11" -N 0d0e0f -n ebdbb2 -M 689d6a -m 0d0e0f -S 689d6a -s 0d0e0f  -p "$label:")
